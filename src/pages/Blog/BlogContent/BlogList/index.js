...
import { getBloglist, getBlogCategories } from '@rsrc/api';
import moment from 'moment';
import { Link } from 'react-router-dom';


export const BlogList = ({ selectedCategoryId }) => {
    ...
    useEffect(() => {
        getBloglist(currentPage, 10, selectedCategoryId)
            .then(response => {
            ...
            {newsData.map(item => (
                <Row className="desk-margin" key={item.id}>
                    ...
                    <Col lg={6} className="news-bg">
                        ...
                        <Link to={`/blog/${item.slug}`} className="text-decoration-none">
